...
class Model(Module):
    def __init__(self, config: ModelCfg):
        ...
        self.policy_head = Linear(config['dim'], 1)
        self.value_head = Linear(config['dim'], 1)
    ...
    def match(self, scope_reprs: Tensor, hole_reprs: Tensor, edge_index: Tensor) -> Tensor:
        ...
        return self.policy_head(self.norm(sources) * self.norm(targets)).squeeze(-1)

    def value(self, hole_reprs: Tensor) -> Tensor:
        return self.value_head(self.norm(hole_reprs)).squeeze(-1)
